...
class DungeonGenerator:
    ...
    def generate_dungeon(self):
        ...
        for _ in range(self.num_rooms):
            room_width = random.randint(self.min_room_width, self.max_room_width)  # Random room width between 5 and 15
            room_height = random.randint(self.min_room_height, self.max_room_height)  # Random room height between 5 and 10
            x = random.randint(0, self.width - room_width)  # Generate random x-coordinate
            y = random.randint(2, self.height - room_height)  # Generate random y-coordinate
...
def process_input():
    ...
    max_room_width = int(request.form['user_input_max_width'])
    min_room_width = int(request.form['user_input_min_width'])
    max_room_height = int(request.form['user_input_max_height'])
    min_room_height = int(request.form['user_input_min_height'])
    overlap = request.form['yes_no']
